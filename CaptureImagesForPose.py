...
def getPoseFolderName():
    # We append the computer name to the calib folder
    hostName = socket.gethostname()
    poseImageDir = hostName + 'PoseImgs'
    return poseImageDir


def poseDirCheck(poseImageDir):

    if os.path.isdir(poseImageDir):
        print(poseImageDir + ' Directory exists! - Deleting Folder and its contents')
        shutil.rmtree(poseImageDir)

    os.mkdir(poseImageDir)
    print("Creating a new folder to store images for Pose " + poseImageDir +" \n")
...
def captureImgs(poseImageDir):
    ...
    #print("Found Intel Real Sense " + device_product_line)
    print(camaraInfo.getCameraInfo() )
    ...
    try:
        while True:
            ...
            if key == ord('q'): # quit
                ...
                cv2.imwrite(poseImageDir + "/Pose_" + str(counter) + ".png", color_image)
                counter = counter + 1
                print("Writing color image " + str(counter) + " to dir:" + poseImageDir)
...
if __name__ == '__main__':
    ...
    poseImageDir = getPoseFolderName()
    saveToJSON('pose','poseInfo',poseImageDir)
    poseDirCheck(poseImageDir)
    captureImgs(poseImageDir)